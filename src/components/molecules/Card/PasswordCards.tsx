  ...
  const onDragEnd = async (result: any) => {
    ...
    try {
      ...
      if (oldGroup !== newGroup) {
        ...
        const oldPasswords = updatedSourcePasswords.map((password, index) => ({
          id: password.id,
          group_id: password.group,
          index,
        }));
        const passwordsToUpdate = updatedDestinationPasswords.map((password, index) => ({
          id: password.id,
          group_id: password.group,
          index,
        }));
        ...
        const passwordsToUpdate = updatedSourcePasswords.map((password, index) => ({
          id: password.id,
          group_id: password.group,
          index,
        }));
            ...
            <StrictModeDroppable key={key} droppableId={key}>
              {(provided) => (
                <div {...provided.droppableProps} ref={provided.innerRef}>
                  {/* Group name */}
                  <div
                    className="group_name flex justify-between items-center p-4 bg-primary rounded cursor-pointer dark:bg-primary-dark"
                    onClick={() => toggleDropdown(key)}
                  >
                    <div className="flex items-center">
                      <h2 className="text-black dark:text-white text-lg font-semibold mr-2">{key}</h2>
                      <span className="text-black dark:text-white mr-2 bg-[#5e9afa] rounded-full px-2 py-1 text-xs dark:bg-accent-dark">
                        {data[key].length}
                      </span>
                    </div>
                    <ExpandLessIcon
                      ...
                      style={{
                        transform: `${dropdownOpenState[key] ? 'rotate(180deg)' : 'rotate(0deg)'}`,
